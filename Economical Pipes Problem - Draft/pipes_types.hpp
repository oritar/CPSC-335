...
// gnomes_types.hpp
//
// Data types for the input and output of the economical pipes problem.
//
...
enum cell_kind { CELL_SOIL, CELL_ROCK, CELL_OPEN };
...
public:
  ...
  : cells_(rows, std::vector<cell_kind>(columns, CELL_SOIL)) {
  ...
  // (0, 0) may only be CELL_SOIL. Other coordinates may be any kind.
  void set(coordinate row, coordinate column, cell_kind kind) {
    assert(is_row_column(row, column));

    if ((row == 0) && (column == 0)) {
      assert(kind == CELL_SOIL);
    }
  ...
  // Create a random grid with the given number of rows, columns, open cells,
  // rock cells, and random number generator. rows and columns must both be
  // positive. The number of open and rock cells must be less than the number
  // of total cells in the grid.
  template <typename URNG>
  static grid random(coordinate rows, coordinate columns,
                     unsigned open_count, unsigned rock_count,
                     URNG&& gen) {

    assert(rows > 0);
    assert(columns > 0);
    assert((open_count + rock_count) < (rows * columns));

    // The output grid, at this point all cells are earth.
    grid result(rows, columns);

    // Next we are going to shuffle all the positions that could be open or 
    // contain the rock, and use the shuffled order to place those cells 
    // randomly. This approach makes it easy to prevent the open/rock cells 
    // from colliding or being placed at (0, 0).
    ...
    // Use the first open_count positions to place open cells.
    assert(positions.size() >= open_count);
    for (unsigned i = 0; i < open_count; ++i) {
      auto& p = positions.back();
      result.set(p.row, p.column, CELL_OPEN);
      positions.pop_back();
    }

    // Use the first rock_count positions to place rock cells.
...
//
// This class tracks the ending position and total open cells of the path, 
// in order to make it easier to compare candidate solutions in 
...
public:
  ...
  // by the algorithms.
  path(const grid& setting, 
       const std::vector<step_direction>& steps_after_start) {
    initialize(setting);
  ...
  unsigned total_open() const { return total_open_; }
  ...
  void add_step(step_direction dir) {
    ...
    // Update final row, column, and total open cells.
    final_row_ = row_after(dir);
    final_column_ = column_after(dir);
    if (setting_->get(final_row_, final_column_) == CELL_OPEN) {
      ++total_open_;
    }
  }
  ...
  std::vector<std::string> printable() const {
    ...
    for (auto& s : steps_) {
      ...
      char c;
      if (setting_->get(row, column) == CELL_OPEN) {
        c = 'S';
      } else {
        c = '+';
      }
  ...
  // Print the path, including the number of steps and open cells in the path.
  void print() const {
    for (auto& line : printable()) {
      std::cout << line << std::endl;
    }
    std::cout << "steps=" << steps_.size()
              << " number of open cells=" << total_open_
              << std::endl;
  }
